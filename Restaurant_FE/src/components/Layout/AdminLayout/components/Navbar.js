...
const navigate = useNavigate()
	...
	async function updateNoti(Token) {
        try {
                const response = await axios.post('http://localhost:8080/api/v12/updatenoti', Token);
	...
	const handleMouseEnter = () => {
	  ...
	  const handleMouseLeave = () => {
		  try {
			  axios.get(`http://localhost:8080/api/v12/sendnoti?token=${cookies.get('AccessTokenAdmin')}`);
		  } catch (error) {
				  alert('Có lói xảy ra vui lòng thử lại')
			  // Xử lý lỗi tại đây.
		  }
 ...
 useEffect(() => {
	if(cookies.get('AccessTokenAdmin') !== undefined) {	
		...
		axios.all([
		  axios.get(`http://localhost:8080/api/v12/shownotifi?token=${cookies.get('AccessTokenAdmin')}`),
		])
