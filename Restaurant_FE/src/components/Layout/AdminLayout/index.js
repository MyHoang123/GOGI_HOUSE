...
import {useCallback, useEffect, useRef, useState, createContext, memo} from 'react';
...
function AdminLayout( {Children} ) {
    ...
    async function getMessageList() {
        try {
                const response = await axios.post(`${process.env.REACT_APP_CALL_API}/api/v12/showallchat`, {
    ...
    const handleClickOpenChat = (listMessage) => {
            if(listMessage.length === 0) {
                getMessageList()
            }
            setChat(true)
        }
    const handleClickUser = (Name,Iduser,socket,listMessage) => {
        if(Name !== null) {
            ...
            <div ref={mainRef} className='main'>
                ...
                <main className='content'>
                    <div className='container-fluid p-0'>
                        <ElementContextAdmin.Provider value={{socket,cookies, socketOrder}}>
                               {Children}
                        ...
                        <div className={cx('massage_content_sidebar')}>
                            <div className={cx('massage_content_sidebar_find')}>
                                ...
                                <div onClick={(e) => handleClickFilterChat(e)} className={cx('massage_content_sidebar_find-filter')}>
                                    <h3 style={{marginBottom:'0', fontSize:'.9vw'}}>Tất cả</h3>
                            ...
                            {listMessage.map((ls,index) => (
                                <div onClick={() => handleClickUser(ls.Name,ls.IdSend,socket,listMessage)} key={index} style={activeUser.Name === ls.Name ? {background:'rgba(0, 0, 0, 0.08)'} : null} className={cx('massage_content_sidebar_list')}>
                                    <div className={cx('massage_content_sidebar_list-avt')}>
                                        <img style={{borderRadius:'50%',width:'35px',height:'35px',objectFit:'cover'}} src={`${process.env.REACT_APP_CALL_API}/api/v12/avtuser/${ls.Avt}`}/>
                                    </div>
