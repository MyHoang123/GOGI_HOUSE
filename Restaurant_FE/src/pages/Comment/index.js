...
function Comment() {
  const cookies = new Cookies()
   ...
   async function showComment(IdProduct) {
    try {
        const response = await axios.get(`http://localhost:8080/api/v12/showcomment?IdProduct=${IdProduct}`);
   ...
   async function filterComment(Comment) {
    try {
        const response = await axios.post('http://localhost:8080/api/v12/filtercomment', Comment);
        if(response.data.massege === "Thanh cong") {
            setComment(response.data.data)
        }
       else {
   ...
   async function createRepComment(Comment) {
    ...
    const handleClickShowComment = (IdProduct,Star) => {
        setStarAllCmt(Star)
    ...
    const handleClickAutoRepCommemt = useCallback(async (e) => {
        if(e.target.checked) {
            const { value: text } = await Swal.fire({
              ...
              if (text) {
                try {
                    const response = await axios.post('http://localhost:8080/api/v12/onautorepcomment',{content: text, status: 1, token: cookies.get('AccessTokenAdmin')});
                    if(response.data.massege === "Thanh cong") {
            ...
            try {
                const response = await axios.post('http://localhost:8080/api/v12/onautorepcomment',{status: 0, token: cookies.get('AccessTokenAdmin')});
                if(response.data.massege === "Thanh cong") {
                  setCheckAutoRep(false)
                }
                else {
    ...
    useEffect(() => {
        axios.all([
            axios.post('http://localhost:8080/api/v12/checkAutoRep',{
                token: cookies.get('AccessTokenAdmin')
            }),
        ])
