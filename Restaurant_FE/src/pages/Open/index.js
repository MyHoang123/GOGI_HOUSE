...
function DragAndDropExample() {
 ...
 async function updataVisible(product) {
  ...
  const handleClickEdit = async (e) => {
    if(e.target.checked) {
      try {
        const response = await axios.post('http://localhost:8080/api/v12/onOpen',{status: 1, token: cookies.get('AccessTokenAdmin')});
        if(response.data.massege === "Thanh cong") {
          setStatus(true)
        }
        else {
      ...
      try {
        const response = await axios.post('http://localhost:8080/api/v12/onOpen',{status: 0, token: cookies.get('AccessTokenAdmin')});
        if(response.data.massege === "Thanh cong") {
          setStatus(false)
        }
        else {
  ...
  useEffect(() => {
    axios.all([
        axios.post('http://localhost:8080/api/v12/checkstatus',{
...
export default memo(DragAndDropExample);