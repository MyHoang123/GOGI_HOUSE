...
exports.showTypeOrder = async (req, res) => {
  Product.getAllTypes((data) => {
    if(data !== null) {
        return res.status(200).json({
            massege: 'thanh cong',
            data: data
...
exports.createType = async (req, res) => {
    ...
    const day = currentDate.getDate();
    const formattedDate = `${year}-${month}/${day}`;
    Product.insertType(Name, Price,2,formattedDate,Id,formattedDate,Id)
    return res.status(200).json({
        massege: 'thanh cong',
     })
}
...
exports.filterCategori = async (req, res) => {
    // const {IdType, IdCate} = req.body
    const IdType = JSON.parse(req.query.IdType)
    const IdCate = JSON.parse(req.query.IdCate)
        if(IdType !== null && IdCate !== null) {
            Product.filterCateType(IdCate,IdType,(data) => {
                if(data !== null) {
                    return res.status(200).json({
                        massege: 'Thanh cong',
                        data: data
        ...
        else if(IdCate !== null && IdType === null) {
            Product.filterCate(IdCate,(data) => {
                if(data !== null) {
                    return res.status(200).json({
                        massege: 'Thanh cong',
                        data: data
