...
const createComment = async (Containt, Star, IdProduct, IdAcc, IdBill, result) => {
     db.query('INSERT INTO comment (Containt, RepComment, Star, IdProduct, IdAcc,IdBill )  VALUE(?,?,?,?,?,?)',[Containt,'', Star, IdProduct, IdAcc,IdBill] , function(err,data){
        if(err || data.length <= 0) {
            result (null)
        }
        else {
...
const showCommentAdmin = async (result) => {
     db.query('SELECT p.Id, p.Name,p.Price,p.Img,p.Sales,c.Star FROM products p INNER JOIN comment c ON p.Id = c.IdProduct GROUP BY p.Id,p.Name,p.Price,p.Img,p.Sales,c.Star',function(err,data) {
        if(err || data.length <= 0) {
            result (null)
        }
        else {
