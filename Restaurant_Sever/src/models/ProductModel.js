...
const insertProduct = async (Name,Price,Img,Dsription,IdCategoris,CreatedDateTime,CreatedUserId,LastModifiedDateTime,LastModifiedUserId,result) => {
    db.query('INSERT INTO products (Name, Price, Img, Dsription, IdCategoris, Sales, Visible,CreatedDateTime, CreatedUserId , LastModifiedDateTime,LastModifiedUserId)  VALUE(?,?,?,?,?,?,?,?,?,?,?)',[Name,Price,Img,Dsription,IdCategoris,0,1,CreatedDateTime,CreatedUserId,LastModifiedDateTime,LastModifiedUserId],function(err,res){
        if(err) {
            console.log("🚀 ~ db.query ~ err:", err)
            result (null)
            return
