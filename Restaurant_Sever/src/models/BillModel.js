...
const getAllPriceBill = async (result) => {
    db.query(`SELECT current_week_count.total_amount AS AllCount, ROUND(((((current_week_count.total_amount) - (last_week_count.total_amountl)) / (last_week_count.total_amountl)) * 100), 2) AS Since_last_week FROM (SELECT SUM(TotalPrice) AS total_amount FROM bill WHERE DATETIME >= NOW() - INTERVAL 1 WEEK) AS current_week_count,(SELECT SUM(TotalPrice) AS total_amountl FROM bill WHERE DATETIME >= NOW() - INTERVAL 2 WEEK) AS last_week_count;`, function(err,data){
        if(err || data.length <= 0) {
            result (null)
        }
        else {
